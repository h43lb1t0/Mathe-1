...
\newcommand*{\verticalline}[3][black]{%
  \begin{tikzpicture}[remember picture, overlay]
    \draw [densely dashed, color=#1] ($(pic cs:#2) + (1ex,1.8ex)$)
      -- ($(pic cs:#3) + (1ex,-0.3ex)$);
  \end{tikzpicture}
}
...
\subsubsection{3x3 Matrix (Sarrus-Regel)}

\[
\begin{bmatrix}
    \tikzmark{a}a & \tikzmark{d}d & \tikzmark{g}g \\
    b & e & h \\
    \tikzmark{c1}c & \tikzmark{f1}f & \tikzmark{i}i
\end{bmatrix}
\begin{matrix}
    \tikzmark{a2}a & \tikzmark{d2}d \\
    b & e \\
    \tikzmark{c}c & \tikzmark{f}f
\end{matrix}
\]
\verticalline[blue]{a}{i}
\verticalline[blue]{d}{c}
\verticalline[blue]{g}{f}
\verticalline[red]{c1}{g}
\verticalline[red]{f1}{a2}
\verticalline[red]{i}{d2}
...
\begin{itemize}
    ...
    \item $A \in \mathbb{R}^{nxn}$ ist inventierbar und $b \in \mathbb{R}^n \quad \rightarrow$ LGS $Ax = b$ hat eine eindeutige Lösung $x = A^{-1}b$ 
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Eliminationsmatrix} Vorzeichen der nicht Diagonalelemente wird invertiert.  
\end{itemize}

\subsubsection{Bei welchen Parameter gibt es keine Inverse?}
Die Determinantenformel $\det A = 0$ der Matrix nach $p$ umstellen. 

\subsection{Vektorraum $\mathbb{R}^2$}
Zeugs über basen und so hier

\subsubsection{Geraden}

\subsubsection{Länge und Abstände}
\[
\text{Sei} \quad \vec{a} = \begin{pmatrix} x \\ y \end{pmatrix} \in \mathbb{R}^2
\]
\[
\text{Länge} \quad \vert \vec{a} \vert = \sqrt{x^2 + y^2}
\]

\subsubsection{Orthogonalität und Winkel}
\end{document}